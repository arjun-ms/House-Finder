...
async def run_pipeline():
    ...
    cnf.RESULTS_JSON = "output/reports_without_project.json"
    cnf.REPORT_MD = "output/reports_without_project.md"
    cnf.REPORT_HTML = "output/reports_without_project.html"
    cnf.MAX_LISTINGS_TO_SCRAPE = 50
    ...
    for attempt in range(max_retries):
        try:
            ...
            if all_scraped:
                original_len = len(all_scraped)
                all_scraped = [p for p in all_scraped if "-pdpid-" not in p.get("listing_url", "")]
                print(f"[*] Dropped {original_len - len(all_scraped)} Project (-pdpid-) pages. {len(all_scraped)} individual listings remain.")
                if not all_scraped:
                    print("[!] No properties left after dropping pdpid pages. Retrying or ending.")
                else:
                    break
        except Exception as e:
            error_str = str(e)
